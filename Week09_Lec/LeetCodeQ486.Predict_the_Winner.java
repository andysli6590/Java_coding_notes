...
public class Solution {
    ...
    public boolean PredictTheWinner(int[] nums) {
        ...
        return helper(nums, 0, nums.length - 1, sum) >= 0;
    }
    
    private int helper(int[] nums, int start, int end, int sum[][]) {
        if (sum[start][end] == Integer.MIN_VALUE) {
            sum[start][end] = start == end ? nums[start] : Math.max(
                nums[start] - helper(nums, start + 1, end, sum),
                nums[end] - helper(nums, start, end - 1, sum));
        }
