...
row (including the leftmost place and rightmost place). Then, if there is a group 
...
public class Solution {
    ...
    public int findMinStep(String board, String hand) {
        ...
        for (int i = 0; i < hand.length(); i++) {
            int index = hand.charAt(i) - 'A';
            handCount[index]++;
        }
        int result = helper(board + "#", handCount); 
        //append a "#" to avoid special process while j == board.length, make the code shoter
    ...
    private int helper(String s, int[] count) {
        ...
        for (int i = 0, j = 0; j < s.length(); j++) {
            ...
            need = 3 - (j - i); //ball need to remove current consecutive balls;
            if (count[s.charAt(i) - 'A'] >= need) {
                count[s.charAt(i) - 'A'] -= need;
                result = Math.min(result, need + helper(s.substring(0, i) + s.substring(j), count));
                count[s.charAt(i) - 'A'] += need;
            }
            i = j;
        }
